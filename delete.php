...
if(isset($_GET['id'])){
	$id = $_GET['id'];
	$cek = mysqli_query($koneksi, "SELECT * FROM barang tenaga kesehatan WHERE id='$id'") or die(mysqli_error($koneksi));
	if(mysqli_num_rows($cek) > 0){
		$del = mysqli_query($koneksi, "DELETE FROM barang tenaga kesehatan WHERE id='$id'") or die(mysqli_error($koneksi));
		if($del){
